...
namespace ChartAndGraph
{
    ...
    public class GraphData : ScrollableChartData, IInternalGraphData
    {
        ...
        public void SetCategoryLine(string category, Material lineMaterial, double lineThickness, MaterialTiling lineTiling)
        {
            ...
            data.LineThickness = lineThickness;
            data.LineTiling = lineTiling;
            //start of new line stuff
            //369 GraphChart.cs
            graphChart.InternalGenerateChart();

            //float widthRatio = trans.rect.width;
            //float heightRatio = trans.rect.height;
            //Rect viewRect = new Rect(0f, 0f, widthRatio, heightRatio);
            //GameObject mask = graphChart.CreateRectMask(viewRect);
            //CanvasLines lines = graphChart.CreateDataObject(data, mask);
        ...
        public void AddPointToCategory(string category, double x,double y, double pointSize = -1f)
        {
            if (mData.ContainsKey(category) == false)
            {
                Debug.LogWarning("Invalid category name. Make sure the category is present in the graph: "+category);
                return;
            }
